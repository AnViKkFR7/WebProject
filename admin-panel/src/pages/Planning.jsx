...
const Planning = () => {
  ...
  const [taskForm, setTaskForm] = useState({ title: '', description: '', client_company_id: '', type: 'todo', start_datetime: '', end_datetime: '', is_all_day: false, recurrence_type: 'none', recurrence_value: null });
  const [billForm, setBillForm] = useState({ title: '', description: '', client_company_id: '', billing_date: '', amount: '', direction: 'cobrar', recurrence_type: 'none', recurrence_value: null });
  ...
  const openTaskModal = () => {
    setTaskForm({ title: '', description: '', client_company_id: selectedCompanies[0]?.id || '', type: 'todo', start_datetime: '', end_datetime: '', is_all_day: false, recurrence_type: 'none', recurrence_value: null });
    setErrorTask('');
    setShowTaskModal(true);
  };
  const openBillModal = () => {
    setBillForm({ title: '', description: '', client_company_id: selectedCompanies[0]?.id || '', billing_date: '', amount: '', direction: 'cobrar', recurrence_type: 'none', recurrence_value: null });
    setErrorBill('');
  ...
  const handleAddCompany = async (e) => {
    ...
    try {
      ...
      const created = await createClientCompany(newCompany);
